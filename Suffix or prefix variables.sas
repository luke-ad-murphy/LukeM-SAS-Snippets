/*************************************************************************************************
 * program: ren_stem.mac
 * programmer: dan blanchette - the carolina population center unc-ch
 * date: 27Apr2004
...
** Let SAS know about the ren_stem macro by including it in your program: **;
 %include(/bigtemp/sas_macros/ren_stem.mac);
...
%macro ren_stem(dset,vars,constant,options);
  ...
  %end;
  %let vt= ;
  %if %index(&options,%str(N ))=1 or %index(&options,%str( N)) %then %do;
   %let vt=N;
  %end;
  %if %index(&options,%str(C ))=1 or %index(&options,%str( C)) %then %do;
   %let vt=C;
  %end;
  %let sascode=0;
