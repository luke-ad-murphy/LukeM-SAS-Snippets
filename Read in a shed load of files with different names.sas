...
X DIR/s/b "J:\WLM files\Latest files\Managed folder\" > "J:\WLM files\Latest files\Managed folder\managed.txt" ;
...
data base;
 length dummy $1000;
  infile "J:\WLM files\Latest files\Managed folder\managed.txt" dlm ='09'x ;
    input dummy $ ;

	if dummy ='J:\WLM files\Latest files\Managed folder\managed.txt' then delete;
run;
...
data _null_;
  set base; 
   ...
   call execute('rundate=rundate+1;'); 
...
/*proc contents data=asis5.wlm_test;
run;*/
